...
const ProgressManager = (() => {
  const STORAGE_KEY = 'jlpt-quiz-progress';
  const VERSION = 1;
  ...
  // identifier scheme — the kanji/compound text itself (see itemText() in
  // app.js) is still the identifying key.
  function gradeKey(mode, grade) {
    return `${MODE_PREFIX[mode] || 'kanji-n'}${grade}`;
  }
